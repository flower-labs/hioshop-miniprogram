...
Page({
  ...
  async getRelationList() {
    ...
    const resp = await util.request(api.GetBabyRelation, { baby_id: defaultBabyId }, 'POST');
    const relationList = (resp.data || []).map(item => ({
      ...item,
      register_time: moment(item.register_time * 1000).format('YYYY-MM-DD HH:mm'),
    }));
